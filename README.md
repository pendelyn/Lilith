...
Chat shows the current browser step, URL, and screenshots as the worker reports them. History on the card uses the same task states as other work: waiting, working, stopped, done, and failed. Stop remains available while a run is streaming. Screenshots are stored under `.lilith-retention/` with the 7-day TTL and served only as `GET /screenshots/:id` with the Bearer token in the Authorization header — never as a query parameter or public static file. The app keeps screenshot ids in chat history and fetches JPEG bytes into memory; those bytes are not written to AsyncStorage. Before each capture Lilith projects a fixed safe surface: password, payment, and named token/credential form controls are masked (value, and matching title/aria-label), canvas/video/iframe pixels are hidden, and the shown URL strips credentials plus known secret query keys including `client_secret`. Approval Operation/Data on the card, `GET /tasks`, and persisted chat use that same redacted projection; the server keeps the exact bound URL for one-time dispatch. Stop stays available while an approved browser job is running. That is not recognition of arbitrary page secrets — ordinary visible HTML text stays in the readout because guessing secrets in free text would be unreliable.

## Browser actions (Issue #20)

Send `Bereite den Formulartext vor: Hallo` to fill the local form fixture and show `Preview: Hallo`. That does not submit. Send `Sende das Formular: Hallo`, `Lade die Formulardatei hoch: Hallo`, `Sende die Formularnachricht: Hallo`, `Kaufe im Formular: Hallo`, or `Klicke unklar im Formular: Hallo` to stop before submit, upload, message, purchase, or an ambiguous click. The card is an `external_effect` one-time approval bound to origin, operation, the prepared text, and a digest of the form controls. **Reject** makes no call. **Approve once** sends one pinned HTTPS POST with the action id as `Idempotency-Key`. A changed DOM or payload invalidates the approval before that POST. Chromium stays on `--network=none` and still cannot POST; the host performs the single request through the existing SSRF client. A completed browser action is shown as executed and cannot be undone. There is no undo control.
...
- Screenshots expire after 7 days once stored under `.lilith-retention/`. The Issue #18 browser worker writes JPEG bytes there; Issue #19 shows them in chat through owner-authenticated `GET /screenshots/:id`.
